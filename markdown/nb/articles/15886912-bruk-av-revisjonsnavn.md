...
- **Dokumentidentifikator:** Av.
- **Resultatet**<br>Når Sarah laster opp `FloorPlan_--.pdf` etterfulgt av `FloorPlan_-a.pdf`, leser plattformen de endrede sekvensmerkelappene for validering, men fjerner dem når filen navngis på arbeidsområdet. Tom og designteamet ser en enkelt dokumentbeholder kalt `FloorPlan` der historiske varianter er stablet som revisjoner uten å forskyve påfølgende tegn.
