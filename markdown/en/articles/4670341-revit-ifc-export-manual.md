...
**[IFC export](#h_38cc715c02) - [Modify Setup](#h_85d4335aad) - [IFC Options](#h_487f3aa176) - [Colors and materials](#h_f458b56011)**
...
Enter the name and location that the exported file will have in the system

Export setup

Choose between the following pre-defined setups:
...
IFC4 Reference View [Unofficial]
...
Click on Modify setup in the export setup part of the export ifc dialogue.

This is where the necessary settings for IFC exports can be changed and custom setups can be created.

This is what the modify setup menu can look like:
...
[General](#h_fb41b895ea) - [Additional Content](#h_ed48fc4387) - [Property Sets](#h_04dd25ffef) - [LoD](#h_11ae63fb7f) - [Advanced](#h_f7b35f27cd) - [Geographic Reference](#h_de5067b34b)
...
These options can change depending on the IFC version that is selected.
...
Before Revit 2026 this option was available in File -> Export -> Options -> IFC Export Options.

This is what the Manage IFC Export Mapping Settings menu can look like:
...
These are all about how room information can be used further.

a. 1st Level - Ex on use: Quantity withdrawals, management, operation and maintenance (FDVU).

b. 2st Level - Ex on use: Energy analysis, light analysis.

**Facility Type**

This option is only available for IFC 4x3
...
Marine Facility (IfcMarineFacility)
...
Export linked files as separate IFC's
...
Ceiling grids are 2D elements and thus not displayed in the Catenda 3D viewer.
...
Exports all Revit Property sets (pset / properties)

Here is an example of a wall exported with this option:

Revit (_Left_) --- Catenda (_Right_)
...
Typical properties that show in the properties menu are:

Constraints, Cross-Section Definition, Dimensions, Structural, Identity Data, Other

Typical properties that show in the Identification menu are:

IFC Parameters

Export standard IFC properties.

Exports calculated quantities of objects.

Export batch lists

Export one-off Property Set
...
You can find this option in the Manage tab -> Coordinates -> Specify Coordinate Base. _Specify Coordinates at Point_

Relocates a model and rotates the model to True North by specifying coordinates for North/South, East/West, and Elevation.

In Revit it is often easier to model on 90 degree angles and you don't want to rotate the whole model. In this case you can rotate True North instead. You will find the option in the Position dropdown below Coordinates in the Manage tab.
...
Here within the _IFC options_ we make the settings for exporting a model to an IFC file. Here you can customize

setup properties for exporting a model to IFC. What was mentioned at the start of this manual is it is not necessary to bring too much information out of the model. Feel free to tick off unnecessarily information before export.
...
The colors that are displayed in Catenda are read from the IFC file that is imported. When the material property of a family is added to the IFC parameters, the color of the material in the material property is added to the IFC and thus displayed in Catenda.

In Revit materials can be found in the material browser:
