# Using topic templates with approvals

> Apply an approval topic template to an approval workflow template
...
1. When a new approval workflow is created. <br>You [create a new approval workflow](https://support.catenda.com/en/articles/12313251-creating-a-new-workflow) template, navigate at the bottom of the page until you reach the "Final approval" section. <br>Here, you can link an approval topic template for each review state (and associate document final status).<br>
...
1. Go through an approval workflow. <br>After all reviews have been made and once the approval workflow is closed, the associated approval topics will be automatically generated and auto-populated with all the fields that were set in the associated topic template settings, accessible from the [topic template settings page](https://support.catenda.com/en/articles/14076010-topic-template-settings-page).<br>For a topic to get generated as the result of the closing of an approval, the approval has to contain a markup. <br>If an approval topic template is configured for the workflow that the approval follows topics get generated when the approval is closed.<br>For each markup in the approval a separate topic is generated.
